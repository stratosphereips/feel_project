...
class ExtractFeatures(object):
    ...
    def create_4_tuples(self, path_to_ssl_log):
        with open(path_to_ssl_log) as ssl_file:
            for ssl_line in ssl_file:
                ...
                try:
                    self.connection_4_tuples[connection_index].add_ssl_flow(
                        conn_log, label
                    )
                except:
                    self.connection_4_tuples[connection_index] = ConnectionFeatures(
                        connection_index
                    )
                    self.connection_4_tuples[connection_index].add_ssl_flow(
                        conn_log, label
                ...
                self.connection_4_tuples[connection_index].add_ssl_log(
                    ssl_line, valid_x509_list, os.path.basename(path_to_ssl_log)
                )
                ...
                self.connection_4_tuples[connection_index].check_certificate_path(
                    x509_lines_arr, is_founded
                )

                # Find trusted root certificates
                if len(x509_lines_arr) > 0:
                    self.connection_4_tuples[connection_index].check_root_certificate(
                        x509_lines_arr, self.root_certs
                    )
                ...
                if "Benign" in label:
                    ...
                    try:
                        ...
                        for alexa_domain in self.alexa_1000_arr:
                            if (
                                alexa_domain in server_name
                                or server_name in alexa_domain
                            ):
            ...
            for line in f:
                ...
                try:
                    if self.connection_4_tuples[connection_index]:
                        try:
                            if self.connection_4_tuples[
                                connection_index
                            ].get_uid_flow_dict()[conn_uid]:
                                pass
                        except:
                            self.connection_4_tuples[connection_index].add_not_ssl_flow(
                                line, label
                            )
                except:
    ...
    # certificate dict
    def put_server_name_to_dict(
        self, ssl_uid, server_name, tuple_index, x509_uids_list, label
    ):
            ...
            if self.x509_dict[uid_x509]:
                ...
                try:
                    if self.certificate_dict[cert_serial]:
                        self.certificate_dict[cert_serial].add_server_name(
                            server_name, label
                        )
                        self.certificate_dict[cert_serial].add_x509_line(x509_line)
                except:
                    self.certificate_dict[cert_serial] = CertificateFeatures(
