...
CNAMES = ["num_flows", "avg_dur", "std_dev_dur", "percent_stdev_dur", "total_size_of_flows_orig", "total_size_of_flows_resp",
            ...
            "label", "detailedlabel", "id.orig_h", "id.resp_h", "id.resp_p", "proto"]
...
def check_arg():
    ...
    parser.add_argument('-z', '--zeekfolder', help='Path to a folder where all log files are.', action='store',
                        required=False)
    parser.add_argument('-Z', '--zeekfolders', help='Multiple captures. Path to a folder where Bro folders are.',
                        action='store', required=False)

    parser.add_argument('-o', '--output', required=False, default="features.csv", help="Name of the ouput file", action='store')
